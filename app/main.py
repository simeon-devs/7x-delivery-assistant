...
def list_sessions():
    c = conn()
    try:
        return [_conversation_row(c, s) for s in
...
def verify_start(sid: str, body: StartVerify):
    ...
    try:
        with c:
            return convo.verify_start(c, sid, body.tracking_number)
...
def verify_confirm(sid: str, body: ConfirmVerify):
    ...
    try:
        with c:
            return convo.verify_confirm(c, sid, body.code)
    except LookupError as e:
        raise HTTPException(404, str(e))
    finally:
        c.close()
